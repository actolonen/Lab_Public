---
title: "Microbial growth in microplates"
author: "Fermlab"
format: html
editor: visual
toc: TRUE
...
Code to organize, plot, and analyze microbial growth in micro-plates measured with either a Bioscreen or Clario star instrument.
...
Function to normalize OD600 readings by subtracting mean OD600 of blank medium at the same time point.

NOTE: The cells and the blanks must have the same "medium" for the subtraction to work correctly.
...
norm_mean = function(growth.blanks, growth.cells)
{
    ...
    growth.blanks = growth.blanks %>%
        group_by(Medium, Hours)%>%
            mutate(OD600_blank = mean(OD600)) %>%
        ungroup;
    
    growth.blanks = growth.blanks %>%
        select(Hours, Medium, OD600_blank) %>%
        distinct();
    
    # subtract mean blank
    growth.cells = left_join(growth.cells, growth.blanks, by = c("Medium", "Hours"));
...
Function to normalize OD600 readings by subtracting OD600 at time zero. Normalization is on a well-by-well basis.
...
Remove contaminated wells from blanks measurements

```{r remove cotnaminated wells from blanks}
...
growth.cells = growth.all %>%
  filter(!grepl("[Bb]lank|[Ee]mpty", Strain));

# Step 1: subtract blanks from each treatment
growth.cells = norm_mean(growth.blanks, growth.cells);

# Step 2: set time zero to OD600 = 0
...
plot.cells = ggplot(growth.cells, aes(x=Hours, y=OD600, group=Well, color=Treatment)) +
  ...
  theme_classic()+
  mytheme+
  guides(color = guide_legend(override.aes = list(size = 4))); # Big points in legend
...
growth_gc = growth.cells %>%
    filter(Strain %in% c("51.24_21")) %>%
    filter(Treatment %in% c("0% butanol"));
